...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		return httpSecurity
				.httpBasic().disable() // httpBasic 비활성화(header에 user name, password를 암호화하지 않은 상태로 주고받기 때문)
				...
				// 인가(권한) 필요한 URL 지정
				.authorizeRequests()
				// .antMatchers("/user/**").access("hasRole('ROLE_USER') or
				// hasRole('ROLE_FARMER') or hasRole('ROLE_ADMIN')")
				// .antMatchers("/farmer/**").access("hasRole('ROLE_FARMER') or
				// hasRole('ROLE_ADMIN')")
